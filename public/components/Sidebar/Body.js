...
import { DOCUMENT_LIST_CLASSNAME, ADD_CLASSNAME, TOGGLE_CLASSNAME, DELETE_CLASSNAME, ROOT_ADD_CLASSNAME, } from '../../constants.js';
...
class Body extends Component {
    ...
    renderTree(document, depth) {
        return `
      <div data-id="${document.id}">
        <div data-id="${document.id}" data-depth="${depth}" class="document-list ${DOCUMENT_LIST_CLASSNAME}"
          style="padding: 2px 14px 2px ${14 * (depth + 1)}px;"
        >
          <div role="button" class="sidebar-btn ${TOGGLE_CLASSNAME} ${document.toggled ? 'toggled' : ''}"></div>
          <div class="document-list-title-wrapper ${Number(location.pathname.split('/')[2]) === document.id
            ? 'current-document'
            : ''}">
            <div class="document-list-title">
      ...
      ${document.documents.length && document.toggled
            ? document.documents
                ...
                .join('')
            : document.toggled
                ? `<div class="document-list" style="padding: 2px 14px 2px ${14 * (depth + 2)}px;">
              <div class="document-list-title-wrapper">
                <div class="document-list-title">
                  하위 문서가 없습니다.
                </div>
