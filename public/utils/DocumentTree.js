...
import { getState, dispatch, SET_DOCUMENT_TREE, SET_CURRENT_DOCUMENT, SET_TOGGLE_CONTROLLER, } from '../core/store.js';
...
class DocumentTree {
    ...
    deleteSubTree(document, deletedId) {
        ...
        for (const child of document.documents) {
            ...
            if (result)
                ret.documents.push(result);
        }
        return ret;
    }
    _toggleTree(id) {
        const nextDocumentTree = JSON.parse(JSON.stringify(getState().documentTree)); // deep copy
    ...
    _deleteFromTree(id) {
        const currentDocumentTree = JSON.parse(JSON.stringify(getState().documentTree));
